...
`bruno-portfolio` is a premium, story-driven personal portfolio website for a
university professor and medical physicist. It replaces a generic institutional
staff page with an editorial experience that explains who he is, what he
researches, why it matters, who he teaches, and how to work with him. Content is
structured local TypeScript data (`data/`) so it can move to a CMS later.
...
Package manager: **bun** (`bun.lock`, `packageManager: bun@1.4.2`). Use `bun`
and `bunx`, not npm or pnpm.

- Install: `bun install`
- Dev server: `bun run dev` (http://localhost:3000, Next.js 16 with Turbopack)
- Build: `bun run build`
- Production server: `bun run start`
- Lint: `bun run lint`
- Typecheck: `bunx tsc --noEmit`
- Add a shadcn/ui component: `bunx --bun shadcn@latest add <component>`
  (style `base-lyra`, built on `@base-ui/react`; components land in
  `components/ui/`)

No test command is configured, so there is no unit-test gate yet. Run `/tests`
or `$tests` to add a runner and record the real test command here. No Verify
command or GitHub checks exist yet; run `/ci` or `$ci` to set them up.
