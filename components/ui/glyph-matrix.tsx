...
interface GlyphMatrixProps extends React.HTMLAttributes<HTMLCanvasElement> {
  /** Characters or short tokens to randomly pick from */
  glyphs?: string | readonly string[]
...
export function GlyphMatrix({
  ...
  useEffect(() => {
    ...
    let last = 0
    let stopped = false
    const alphabet = Array.from(glyphs)
    ...
    const resize = () => {
      ...
      cells = new Array(cols * rows).fill(0).map(() => pickGlyph())
