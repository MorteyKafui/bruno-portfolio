...
/**
 * Drag-to-compare between the MRI and synthetic CT renderings. The control is
 * a real range input laid over the plates, so keyboard and screen-reader users
 * get the native slider; the visible divider follows it on a spring.
 */
  ...
  return (
    <figure className={cn("group/scan flex flex-col gap-4", className)}>
      <div className="relative aspect-[4/3] overflow-hidden rounded-md bg-black shadow-[0_30px_80px_-40px_rgb(0_0_0/0.8)] ring-1 ring-foreground/10 select-none">
        ...
        >
          <span className="absolute top-1/2 start-1/2 flex h-11 w-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-white/40 bg-charcoal/70 text-white backdrop-blur-sm transition-transform duration-300 group-hover/scan:scale-105 group-focus-within/scan:ring-2 group-focus-within/scan:ring-teal-bright">
        ...
        <motion.span
          ...
          className="text-eyebrow pointer-events-none absolute top-4 start-4 rounded-full bg-charcoal/60 px-3 py-1.5 text-white/90 backdrop-blur-sm"
        >
          {labels.ct}
        </motion.span>
        <motion.span
          ...
          className="text-eyebrow pointer-events-none absolute top-4 end-4 rounded-full bg-charcoal/60 px-3 py-1.5 text-white/90 backdrop-blur-sm"
        >
          {labels.mri}
        </motion.span>
        ...
        <span
          aria-hidden
          className="text-eyebrow pointer-events-none absolute bottom-4 start-1/2 -translate-x-1/2 text-white/60 transition-opacity duration-500 group-hover/scan:opacity-0"
        >
          {labels.hint}
        </span>
      ...
      <figcaption className="text-eyebrow text-muted-foreground">{labels.illustrative}</figcaption>
    </figure>
  );
}
